...
3. Ответьте на вопросы на экране компьютера.
...
7. Файл financialcsvxlsx.py:
    * csv_reader - принимает на вход строку, путь к файлу, возвращает список словарей, с данными о транзакциях из файла 
    с расширением csv, реализована функция с помощью библиотеки csv.
    * xlsx_reader - принимает на вход строку, путь к файлу, возвращает список словарей, с данными о транзакциях из файла
    с расширением xlsx, реализованна функция с помощью библиотеки pandas.
8. Файл Users.py:
    * number - не принимает никаких параметров, но дает пользователю выбрать один из трех файлов (json, csv или xlsx) и 
    возвращает в результате своей работы список словарей из указанного файла.
    * status - принимает в качестве параметра список словарей из number и сортирует его по статусу операции.
    * ad_questions - принимает в качестве параметра список словарей из функции status и производит основную сортировку задавая
    4 вопроса пользователю: сортировать ли по дате? по возрастанию или убыванию? выводить только транзакции в рублях? 
    сортировать ли по какому-либо слову? Возвращает так же список словарей, но уже отсортированный.
    * result_main - принимает в качестве параметра список словарей после сортировки в функции ad_questions, формирует список 
    из сгруппированных в удобной для формирования вывода в main форме.
9. main.py:
    main - реализация главной функции в проекте, которая вызывает функции из других модулей и связывает работу проекта 
    воедино. Ничего не принимает в качестве параметров, выводит на экран список словарей после сортировками вышеописанными 
    функциями или, в случае если список не удалось сформировать по заданным параметрам - строку.
